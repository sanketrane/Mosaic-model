...
## import data
count_results <- read.csv(file = "output_files/CountsResults.csv") %>%
  group_by(Time) %>%
  summarise(
...
ggplot(count_results) +
  geom_line(aes(x=Time, y = counts, col=popln))+
  scale_y_log10()
...
naidis %>%
  ...
  geom_line() +
  geom_ribbon(aes(x=age_id, ymin=0, ymax=counts, fill=factor(Time)), alpha=0.2)+ facet_wrap(.~ Time)
...
memfast %>%
  ...
  facet_wrap(.~Time) +
  xlim(10, 14.25) + ylim(2, 26)+ my_theme
...
memslow %>%
  ...
  facet_wrap(.~Time) +
  xlim(12.5, 14) + ylim(2, 18)+ my_theme
